...
def check_expiry(accountname):
    with open(f'{ACCOUNT_FOLDER}/TK_cookies_{accountname}.json', 'r') as file:
...
async def run_javascript(proxy_data=None):
    ...
    try:
        ...
        result = type('Result', (), {})()  # Tạo object giả giống subprocess.CompletedProcess
...
async def detect_redirect(page):
...
async def get_image_src(page):
    image_url = await page.get_attribute("img#captcha-verify-image", "src")
    return image_url
...
async def click_on_objects(page, object_coords):
...
async def upload_tiktok(video, description, accountname, hashtags=None, sound_name=None, sound_aud_vol='mix', schedule=None, day=None, copyrightcheck=False, suppressprint=False, headless=True, stealth=False, proxy=None, user_agent=None, logger=None):
    ...
    user_agent (str)(opt) -> custom user agent
    logger (logging.Logger)(opt) -> logger object for logging
    --------------------------------------------------------------------------------------------------------------------------------------------
    """
    try:
        check_for_updates()
    ...
    if os.path.exists(f'{ACCOUNT_FOLDER}/TK_cookies_{accountname}.json'):
        cookies, cookie_read = read_cookies(cookies_path=f'{ACCOUNT_FOLDER}/TK_cookies_{accountname}.json')
        expired = check_expiry(accountname=accountname)
        if expired == True:
            os.remove(f'{ACCOUNT_FOLDER}/TK_cookies_{accountname}.json')
    ...
    if cookie_read == False:
        ...
        await run_javascript(proxy_data=proxy)
        os.rename('TK_cookies.json', f'{ACCOUNT_FOLDER}/TK_cookies_{accountname}.json')

        cookies, cookie_read = read_cookies(f"{ACCOUNT_FOLDER}/TK_cookies_{accountname}.json")
        if cookie_read == False:
    ...
    async with async_playwright() as p:
        ...
        if captcha == True:
            image = await get_image_src(page)
            if image:
                if suppressprint == False:
                ...
                while solved == False:
                    ...
                    if 'Select 2 objects that are the same' in question or 'Select two objects that are the same' in question:
                        ...
                        imageweb = await page.locator('#captcha-verify-image')
                        imageweb.wait_for()
                        ...
                        await click_on_objects(page, webpage_coords)
        ...
        if sound_name != None:
            ...
            if sound_fail == False:
                ...
                await page.fill(f"input.search-bar-input", f"{sound_name}")
                asyncio.sleep(0.2)
                if stealth == True:
                        asyncio.sleep(2)
                ...
                try:
                    ...
                    asyncio.sleep(0.5)
                    sliders = await page.locator("input.scaleInput")
            ...
            if copyrightcheck == True:
                ...
                while copyrightcheck == True:
                    ...
                    if await page.locator("span" ,has_text="No issues detected.").is_visible():
                        if suppressprint == False:
                            logger.info("Copyright check complete")
                        break
            ...
            try:
                if schedule == None:
                    ...
                    try:
                        ...
                        await page.wait_for_url(url='https://www.tiktok.com/tiktokstudio/content', timeout=2000)
                    except:
                        await page.click('button:has-text("Post")[aria-disabled="false"]', timeout=2000)
                        try:
                            await page.wait_for_url(url='https://www.tiktok.com/tiktokstudio/content', timeout=2000)
                        except Exception as e:
                            logger.warning("POSSIBLE ERROR: Cannot confirm if uploaded successfully, Please check account in a minute or two to confirm")
                            logger.warning("POSSIBLE ERROR: {}".format(e))
                            return False
                    uploaded = False
                    checks = 0
